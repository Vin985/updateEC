...
generateHexGrid <- function(area, proj, hex_proj) {
  area <- spTransform(area, CRS(hex_proj))
  gridArea <- gBuffer(area, width = 100000)
  set.seed(111)
  grid <- spsample(gridArea,
                   n = 1000,
                   type = "hexagonal",
                   cellsize = 50000)
  grid <- HexPoints2SpatialPolygons(grid)
...
create.grid <-
  function(Latitude = c(45, 52),
    ...
    long.dist1 <-
      abs(matrix(c(Longitude[1], Latitude[1]), ncol = 2) - destPoint(
    ...
    long.dist2 <-
      abs(matrix(c(Longitude[2], Latitude[1]), ncol = 2) - destPoint(
    ...
    if (hexgrid) {
      ...
      grid.pts <- spTransform(grid.pts, CRS(prj))

      grid.shp <- hexgrid(grid.pts, convex = TRUE, width = 50000, seed = 111)
      names(grid.shp) <- "ID"
      grid.shp <- spTransform(grid.shp, projection)
      # hexgrid <- spsample(grid.shp,
      #                     n = 10000,
      #                     type = "hexagonal", cellsize = max(degree.dist))
      # grid.shp <- HexPoints2SpatialPolygons(hexgrid)
    } else {
