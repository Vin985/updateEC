...
serverAppCode <- function() {
  ...
  #==============================================================================================================================================
  ## 
  sendAlertNotificationMessageToClient <- function(type = 'codeAction', action = 'start', langage= "fr", operation, tabID){
  ...
  # copy the original files of the analysis code to a tmp dir
  tmp.dir <- file.path(root.directory, "tmp")
  origine.dir <- file.path(root.directory, "origine")
  
  # create the directory 
  dir.create(tmp.dir)
  
  # send the content of the file to the web client via JavaScript
  #   values <- reactiveValues(tab1 = "tab1")
  #   
  #   tab1Obs <- observe({    
  #     values$tab1
  #     
  #     # overwrite the old files in the tmp dir with those in the original directory: 
  #     file.copy(from=list.files(origine.dir, full.names=TRUE), to=tmp.dir, 
  #               overwrite = TRUE, recursive = FALSE, 
  #               copy.mode = TRUE)
  #     
  #     lapply(list.files(tmp.dir, full.names=TRUE), sourceIt)
  #     
  #     sendRCodeACEToClient("tab1",  paste0(tmp.dir, "/tab1.R"))
  #     
  #   })
  
  #=============================================================================================================================================
  # send the content of the file to the web client via JavaScript
  sendRCodeACEToClient <- function(target, filename){
    session$sendCustomMessage(type = 'shinyace',
                              message = list(target = target, # 1, 2, ...
                                             content = readRFile(filename)
                              )
    )
  }
  ...
  initCodeTabs <- function(tabNbr){
    ...
    #lapply(list.files(tmp.dir, full.names=TRUE), sourceIt)
    sourceIt(file.path(tmp.dir, paste0("tab",tabNbr,".R")))
    sendRCodeACEToClient(tabNbr,  paste0(tmp.dir, "/", paste0("tab", tabNbr), ".R"))
  }
  ...
  #==============================================================================================================================================
  #
  tabReactiveValue <- reactiveValues(tab = "tab1")
  ...
  for (i in 1:4){
    local({
      ...
      # save observers
      #---------------------------------------------------------------------------
      assign(paste0("obsSaveCodeTab",ii),  observe({
        
        inputTabAction <- paste0("tab",ii,"SaveCodeButton")
        inputTabCode   <- paste0("tabCode",ii)
        input[[inputTabAction]]
        
        isolate({
          
          sendAlertNotificationMessageToClient(type = 'codeAction', 
                                               tabID = paste0("tabCodeAlert",ii), 
                                               operation = ifelse(input$language=="fr","Sauvegarde du code",
                                                                  "Saving code"),
                                               langage = input$language, 
                                               action = 'start'
          )
          
          codeTab <- input[[inputTabCode]]
          
          if(is.null(codeTab)) 
            return(NULL)
          
          # print(paste0("input$tabSaveCodeButton", ii, "!=0"))
          
          writeRFile(paste0(tmp.dir, "/tab",ii,".R"), codeTab)
          tabReactiveValue$tab = paste0("tab",ii)
      ...
      # reload observers
      #---------------------------------------------------------------------------
      assign(paste0("obsReloadCodeTab",ii),  observe({
        
        inputTabAction <- paste0("tab",ii,"ReinitCodeButton")
        inputTabCode   <- paste0("tabCode",ii)
        input[[inputTabAction]]
        ...
        isolate({
          ...
          sendAlertNotificationMessageToClient(type = 'codeAction', 
                                               tabID = paste0("tabCodeAlert",ii), 
                                               operation = ifelse(input$language=="fr","Chargement du code",
                                                                  "Loading code"),
                                               langage = input$language, 
                                               action = 'start'
          )
          
          #print(paste0("input$tabReloadCodeButton", ii, "!=0"))
          # make a copy of the original file      
          file.copy(from=file.path(origine.dir, paste0("tab",ii,".R")), to=tmp.dir, 
                    overwrite = TRUE, recursive = FALSE, copy.mode = TRUE)
          # sourceIt(paste0(tmp.dir, "/tab1.R")) # only if the reload will also be responsable of sourcing the file 
          sendRCodeACEToClient(ii, paste0(tmp.dir, "/tab",ii,".R"))
      ...
      #---------------------------------------------------------------------------
      # initialise observers (source the coresponding R code)
      #---------------------------------------------------------------------------
      assign(paste0("obsInitCodeTab",ii),  observe({
        
        inputTabAction <- paste0("tab",ii,"IntegrateCodeButton")
        inputTabCode   <- paste0("tabCode",ii)
        input[[inputTabAction]]
        ...
        isolate({
          ...
          sendAlertNotificationMessageToClient(type = 'codeAction', 
                                               tabID = paste0("tabCodeAlert",ii), 
                                               operation = ifelse(input$language=="fr","Intégration du code",
                                                                  "Integrating code"),
                                               langage = input$language, 
                                               action = 'start'
          )
          
          #print(paste0("input$tabReInitCodeButton", ii, "!=0"))
          sourceIt(paste0(tmp.dir, "/", tabReactiveValue$tab, ".R")) #sourceIt(paste0(tmp.dir, "/tab1.R"))
  ...
  detectionInput  <- reactive({
    if(is.null(input$detection )) # pas d'analyse de distance
      return("All")
  ...
  #==============================================================================================================================================
  #
  HaversineFormulaUSingRadians <- function(lat1, lat2, long1, long2) {
    R <- 6371
  ...
  #==============================================================================================================================================
  #
  HaversineFormulaUSingDegrees <- function(lat1, lat2, long1, long2) {
  ...
  nbrRowsOfCreateGridInvalid <-
    function(lat1, lat2, long1, long2, size, max = 999) {
      ...
      lat.dist1 <- abs(matrix(c(Longitude[1], Latitude[1]), ncol = 2) -
                         destPoint(
                           c(Longitude[1], Latitude[1]),
                           b = 180,
                           d = Grid.size[2],
      ...
      lat.dist2 <- abs(matrix(c(Longitude[2], Latitude[1]), ncol = 2) -
                         destPoint(
                           c(Longitude[2], Latitude[1]),
                           b = 0,
                           d = Grid.size[2],
  ...
  datasetVariableNamesReactive <- reactive({
    ...
    isolate({
      ...
      if (input$dataset == "FALSE") {
        ...
        csvFile.variable.names <- names(dataset.embedded.load(dataset.name = reactiveEmbeddedDataset()))
        # sendLoadingMessageToClient(type = 'displayLoading',
        #                            text = "Loading",
        #                            action = 'end')
      }
  ...
  output$densities <- renderUI({
    ...
    isolate({
      ...
      if (input$stratification %in% c("stratgrille", "stratzone")) {
        if (is.null(reactiveSubsetSelectedVariableValue()) || allSpecies()) {
                        ...
                        div(class = "col-md-12",
                            h4(
                              ifelse(
                                input$language == "fr",
                                #encode(paste0(paste0("Données de densités de \"", subset.names[i]),"\" : ")),
                                paste0("Données de densités de \"", subset.names[i], "\" : "),
  ...
  output$distanceData <- renderUI({
    ...
    isolate({
      ...
      pdf.file.name <- substr(pdf.file.name, 3, nchar(pdf.file.name))
...
inputId
  ...
  output$breakClasses <- renderUI({
    ...
    if(!is.null(input$distanceAnalysis) & input$distanceAnalysis == "FALSE"){# it is important as some browsers do not lock the input field to the maximum value 
      nbr.break.classes <- 2
    }
  ...
  density2.map  <- function(resumePlotID = NULL,
    ...
    isolate({
      ...
      if(!is.null(input$distanceAnalysis) & input$distanceAnalysis == "FALSE"){
        titre <- ifelse(input$language == "fr", "Densités non corrigées" , "Uncorrected densities")
      }
      else 
      {
        titre <- ifelse(input$language == "fr", "Densités corrigées" , "Corrected densities")
      }
